...
4. [Lighthouse](https://github.com/GoogleChrome/lighthouse) runs in the same browser session measuring performance and SEO
5. Five scores are calculated:
   ...
   - **Performance** — Lighthouse Core Web Vitals (LCP, FCP, CLS, TBT)
   - **SEO** — Lighthouse SEO checks
6. An **overall score** is computed from all dimensions
7. Google Gemini translates each raw violation into a plain-English explanation
8. The result is saved to Supabase and given a **shareable permalink**
...
axe-core only covers accessibility. Lighthouse covers performance, SEO, and best practices. Running both in the same Puppeteer session means one browser launch gives a complete picture of a site's health — no extra cost, no extra latency.
